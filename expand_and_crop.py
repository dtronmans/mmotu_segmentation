...
def crop_and_expand_image(image_path, binary_mask, transform, expansion_factor=1.2):
    # Step 1: Get the binary mask from the segmentation model
    # _, binary_mask = infer(image, model, transform, histogram=True)
    ...
    opened_image = Image.open(image_path)  # Ensure grayscale
...
if __name__ == "__main__":
    model = UNet(1, 1)
    model.load_state_dict(torch.load("mmotu_40.pt", weights_only=True, map_location=torch.device("cpu")))
    model.eval()
    ...
    for image in os.listdir(images_path):
        cropped_image = crop_and_expand_image(os.path.join(images_path, image), model, transform)
        print()
